...
import { UserChallenge, IUserChallenge } from '../models/UserChallenge';
...
export class ChallengeService {
   ...
   */
  static async generateDailyChallenge(date: Date = new Date()): Promise<IChallenge> {
   ...
   */
  static async getUserDailyChallenge(userId: mongoose.Types.ObjectId): Promise<{
    ...
    if (shouldUpdate && progressIncrement > 0) {
      ...
      if (userChallenge.progress.target > 0) {
        userChallenge.progress.percentage = Math.min(
          100,
          Math.floor((userChallenge.progress.current / userChallenge.progress.target) * 100)
        );
      }
   ...
   */
  static async checkAndAwardBonusBadge(
    userId: mongoose.Types.ObjectId,
  ...
  static async getChallengeHistory(
    userId: mongoose.Types.ObjectId,
    limit: number = 10
  ): Promise<Array<{ challenge: IChallenge; userChallenge: IUserChallenge }>> {
    ...
    const history = userChallenges
      ...
      .map((uc) => ({
        challenge: uc.challengeId as unknown as IChallenge,
        userChallenge: uc,
      }));
   ...
   */
  static async getChallengeStats(userId: mongoose.Types.ObjectId): Promise<{
