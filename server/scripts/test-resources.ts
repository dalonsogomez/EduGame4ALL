  ...
  try {
    ...
    if (passed) {
      ...
      logTest(
        'Get All Jobs',
        hasRequiredFields,
        `Found ${response.data.jobs.length} jobs with match scores`
      );
...
const testGetJobsWithLocationFilter = async () => {
  try {
    ...
    if (passed) {
      const allMatchLocation = response.data.jobs.every((job: any) =>
        job.location.toLowerCase().includes('new york')
      );
      logTest(
        'Get Jobs with Location Filter',
        allMatchLocation,
        `Found ${response.data.jobs.length} jobs in New York`
      );
...
const testGetJobsWithJobTypeFilter = async () => {
  try {
    ...
    if (passed) {
      const allMatchType = response.data.jobs.every((job: any) => job.jobType === 'Full-time');
      logTest(
        'Get Jobs with Job Type Filter',
        allMatchType,
        `Found ${response.data.jobs.length} full-time jobs`
      );
      return allMatchType;
    } else {
...
const testMatchScorePersonalization = async () => {
  try {
    ...
    if (passed && response.data.jobs.length > 0) {
      const hasMatchScores = response.data.jobs.every(
        (job: any) => typeof job.matchScore === 'number' && job.matchScore >= 0 && job.matchScore <= 100
      );
      logTest(
        'Match Score Personalization',
        hasMatchScores,
        `All jobs have valid match scores (0-100)`
      );
...
const testGetGrants = async () => {
  try {
    ...
    if (passed) {
      ...
      const hasRequiredFields =
        grant.title && grant.amount && grant.deadline && Array.isArray(grant.eligibility);

      logTest(
        'Get All Grants',
        hasRequiredFields,
        `Found ${response.data.grants.length} grants`
      );
      return hasRequiredFields;
    } else {
  ...
  try {
    ...
    if (passed) {
      ...
      logTest(
        'Get All Services',
        hasRequiredFields,
        `Found ${response.data.services.length} services`
      );
      return hasRequiredFields;
    } else {
...
const testGetServicesWithTypeFilter = async () => {
  try {
    ...
    if (passed && response.data.services.length > 0) {
      const allMatchType = response.data.services.every(
        (service: any) => service.serviceType === 'Legal'
      );
      logTest(
        'Get Services with Type Filter',
        allMatchType,
        `Found ${response.data.services.length} legal services`
      );
      return allMatchType;
    } else {
  ...
  try {
    ...
    if (passed) {
      ...
      logTest(
        'Get All News',
        hasRequiredFields,
        `Found ${response.data.news.length} news articles`
      );
      return hasRequiredFields;
    } else {
...
const testGetNewsWithCategoryFilter = async () => {
  try {
    ...
    if (passed && response.data.news.length > 0) {
      const allMatchCategory = response.data.news.every(
        (article: any) => article.category === 'important'
      );
      logTest(
        'Get News with Category Filter',
        allMatchCategory,
        `Found ${response.data.news.length} important news articles`
      );
...
const testGetNewsWithLimit = async () => {
  try {
    ...
    logTest(
      'Get News with Limit',
      passed,
      `Returned ${response.data.news.length} articles (limit: 2)`
    );
    return passed;
  } catch (error: any) {
