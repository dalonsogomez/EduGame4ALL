...
async function runTests() {
  ...
  try {
    ...
    try {
      ...
      logTest(
        'Get Initial XP Profile',
        isInitialState,
        `Total XP: ${profile.totalXP}, Level: ${profile.level.currentLevel}`
      );
    } catch (error: any) {
    ...
    try {
      ...
      for (const xp of testXPs) {
        ...
        console.log(
          `   XP ${xp} → Level ${levelInfo.currentLevel} (${levelInfo.progressPercentage.toFixed(1)}% to next)`
        );
    ...
    try {
      ...
      logTest(
        'Award Total XP',
        result.totalXP === 150,
        `Awarded 150 XP. Level: ${result.oldLevel} → ${result.newLevel}${leveledUp ? ' (LEVELED UP!)' : ''}`
      );
    } catch (error: any) {
    ...
    try {
      ...
      logTest(
        'Award Language XP',
        result.categoryXP === 100,
        `Language XP: ${result.categoryXP}, Level: ${result.newCategoryLevel}`
      );
    } catch (error: any) {
    ...
    try {
      ...
      logTest(
        'Award Culture XP',
        result.categoryXP === 75,
        `Culture XP: ${result.categoryXP}, Level: ${result.newCategoryLevel}`
      );
    } catch (error: any) {
    ...
    try {
      ...
      logTest(
        'Award Soft Skills XP',
        result.categoryXP === 50,
        `Soft Skills XP: ${result.categoryXP}, Level: ${result.newCategoryLevel}`
      );
    } catch (error: any) {
    ...
    try {
      ...
      console.log(
        `   Language: ${profile.skills.language.currentXP} XP (Level ${profile.skills.language.currentLevel})`
      );
      console.log(
        `   Culture: ${profile.skills.culture.currentXP} XP (Level ${profile.skills.culture.currentLevel})`
      );
      console.log(
        `   Soft Skills: ${profile.skills.softSkills.currentXP} XP (Level ${profile.skills.softSkills.currentLevel})`
      );
      console.log(
        `   Progress to next level: ${profile.level.progressPercentage.toFixed(1)}%`
      );
    ...
    try {
      ...
      if (hasLeaderboard) {
        ...
        data.leaderboard.slice(0, 3).forEach((entry: any) => {
          console.log(
            `   #${entry.rank} - ${entry.userId.name}: ${entry.xp} XP (Level ${entry.level})`
          );
        });
      }
      ...
      logTest(
        ...
        hasLeaderboard,
        `Found ${data.leaderboard.length} players`
      );
    } catch (error: any) {
    ...
    try {
      ...
      logTest(
        ...
        hasLeaderboard,
        `Category: ${data.category}, Entries: ${data.leaderboard.length}`
      );
    } catch (error: any) {
    ...
    try {
      ...
      logTest(
        'Automatic Level-Up',
        leveledUp,
        `Level: ${result.oldLevel} → ${result.newLevel} (${result.totalXP} total XP)`
      );

      if (result.badgesEarned && result.badgesEarned.length > 0) {
        console.log(`   🏅 Badges earned: ${result.badgesEarned.map((b: any) => b.title).join(', ')}`);
      }
    } catch (error: any) {
