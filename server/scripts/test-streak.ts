...
async function runTests() {
  ...
  try {
    ...
    try {
      ...
      logTest(
        'User registration',
        registerResponse.status === 200 && !!authToken,
        `User ID: ${userId}`
      );
    } catch (error: any) {
    ...
    try {
      ...
      logTest(
        'Initial streak info',
        passed,
        `Streak: ${streakData.currentStreak}, Active today: ${streakData.isActiveToday}`
      );
    ...
    try {
      ...
      if (games.length === 0) {
        ...
        const sessionData = {
          ...
          maxScore: 100,
          timeSpent: 120,
          answers: game.questions.map((q: any, idx: number) => ({
            questionIndex: idx,
        ...
        const sessionResponse = await axios.post(
          `${API_URL}/api/games/${game._id}/sessions`,
          sessionData
        );
        ...
        logTest(
          ...
          `XP earned: ${sessionResponse.data.xpEarned}`
        );
        ...
        logTest(
          'Streak after game play',
          streakAfterData.currentStreak >= 0 && streakAfterData.isActiveToday === true,
          `Streak: ${streakAfterData.currentStreak}`
        );
    ...
    try {
      ...
      logTest(
        ...
        updateResponse.status === 200 && updateData.streak !== undefined,
        `Streak: ${updateData.streak}, Message: ${updateData.message}`
      );
    ...
    try {
      ...
      logTest(
        'Login successful',
        loginResponse.status === 200 && !!newAuthToken,
        'Streak should be updated on login'
      );
      ...
      logTest(
        'Streak after login',
        streakAfterLogin.status === 200,
        `Current streak: ${streakAfterLogin.data.currentStreak}`
      );
    ...
    try {
      ...
      logTest(
        ...
        'Dashboard should reflect updated streak'
      );
    ...
    try {
      ...
      logTest(
        'Reset own streak',
        resetResponse.status === 200 && resetData.streak === 0,
        `Streak after reset: ${resetData.streak}`
      );
      ...
      logTest(
        'Verify streak reset',
        verifyReset.data.currentStreak === 0,
        `Verified streak: ${verifyReset.data.currentStreak}`
      );
    } catch (error: any) {
    ...
    try {
      ...
      logTest(
        ...
        `Total users: ${statsData.totalUsers}, Avg streak: ${statsData.averageStreak}`
      );
    ...
    try {
      ...
      logTest(
        'Reset expired streaks (admin)',
        resetExpiredResponse.status === 200,
        `Checked: ${resetExpiredData.totalChecked}, Reset: ${resetExpiredData.streaksReset}`
      );
