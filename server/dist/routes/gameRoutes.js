...
router.get('/sessions', requireUser(ALL_ROLES), async (req, res) => {
    try {
        if (!req.user?._id) {
...
router.get('/sessions/:sessionId', requireUser(ALL_ROLES), async (req, res) => {
    try {
...
// Request: { id: string }
// Response: { game: Game }
router.get('/:id', requireUser(ALL_ROLES), async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/:id/session', requireUser(ALL_ROLES), async (req, res) => {
    try {
        const { id } = req.params;
