...
router.get('/badges', requireUser(ALL_ROLES), async (req, res) => {
    try {
        if (!req.user?._id) {
...
router.get('/history', requireUser(ALL_ROLES), async (req, res) => {
    try {
        if (!req.user?._id) {
            return res.status(401).json({ error: 'User not authenticated' });
        }
...
router.get('/weekly-report', requireUser(ALL_ROLES), async (req, res) => {
    try {
        if (!req.user?._id) {
            return res.status(401).json({ error: 'User not authenticated' });
        }
