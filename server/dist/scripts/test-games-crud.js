...
async function testGetAllGames() {
    try {
        ...
        if (response.data.games && Array.isArray(response.data.games)) {
            ...
            const activeGames = response.data.games.filter((g) => g.isActive);
            const inactiveGames = response.data.games.filter((g) => !g.isActive);
...
async function testFilterByCategory() {
    try {
        ...
        if (response.data.games && Array.isArray(response.data.games)) {
            ...
            const allLanguageGames = response.data.games.every((g) => g.category === 'language');
            if (allLanguageGames) {
...
async function testFilterByDifficulty() {
    try {
        ...
        if (response.data.games && Array.isArray(response.data.games)) {
            ...
            const allDifficulty2 = response.data.games.every((g) => g.difficulty === 2);
            if (allDifficulty2) {
...
async function testUpdateGame() {
    try {
        ...
        if (response.data.game) {
            ...
            if (response.data.game.title === updateData.title &&
...
async function testFilterByCategoryAndDifficulty() {
    try {
        ...
        if (response.data.games && Array.isArray(response.data.games)) {
            ...
            const allMatch = response.data.games.every((g) => g.category === 'culture' && g.difficulty === 3);
...
async function testDeletedGameNotInListing() {
    try {
        ...
        if (response.data.games && Array.isArray(response.data.games)) {
            const deletedGameFound = response.data.games.some((g) => g._id === createdGameId);
            if (!deletedGameFound) {
...
async function testDeletedGameInAdminListing() {
    try {
        ...
        if (response.data.games && Array.isArray(response.data.games)) {
            const deletedGame = response.data.games.find((g) => g._id === createdGameId);
...
//# sourceMappingURL=test-games-crud.js.map