    ...
    try {
        ...
        try {
            ...
            if (rewards.length > 0) {
                ...
                logTest('Get All Rewards', true, `Found ${rewards.length} rewards. First reward: ${rewards[0].title} (${rewards[0].xpCost} XP)`);
            }
            else {
        ...
        try {
            ...
            const allCorrectCategory = categoryRewards.every((r) => r.category === 'gift-cards');
            logTest('Filter Rewards by Category', allCorrectCategory, `Found ${categoryRewards.length} gift-cards rewards`);
        }
        catch (error) {
        ...
        try {
            await axios.post(`${BASE_URL}/api/rewards/${rewardId}/redeem`, {}, {
                headers: { Authorization: `Bearer ${authToken}` },
            });
        ...
        try {
            ...
            if (gamesRes.data.games && gamesRes.data.games.length > 0) {
                ...
                console.log(`[Test] Playing ${Math.min(sessionsNeeded, 10)} game sessions to earn XP (${game.xpReward} XP per session)...`);
                for (let i = 0; i < Math.min(sessionsNeeded, 10); i++) {
                    await axios.post(`${BASE_URL}/api/games/${game._id}/session`, {
                        score: 100,
                        maxScore: 100,
                        timeSpent: 60,
                        answers: [],
        ...
        try {
            const redeemRes = await axios.post(`${BASE_URL}/api/rewards/${rewardId}/redeem`, {}, {
                headers: { Authorization: `Bearer ${authToken}` },
            });
            ...
            const hasRequiredFields = userReward._id &&
            ...
            if (hasQRCode && hasRequiredFields) {
                ...
                logTest('Redeem Reward', false, `Missing fields: QR=${hasQRCode}, Fields=${hasRequiredFields}`);
            }
        }
        catch (error) {
        ...
        try {
            ...
            const reward = allRewardsRes.data.rewards.find((r) => r._id === rewardId);
            if (reward) {
                logTest('Inventory Management', true, `Available quantity: ${reward.availableQuantity} (decreased by 1)`);
            }
            else {
        ...
        try {
            ...
            const hasRedeemedReward = myRewards.some((r) => r.id === redeemedRewardId);
            const allHaveQRCodes = myRewards.every((r) => r.qrCode && r.qrCode.startsWith('data:image/png;base64'));
            if (hasRedeemedReward && allHaveQRCodes) {
                logTest('Get My Rewards', true, `Found ${myRewards.length} redeemed rewards with QR codes`);
            }
            else {
                logTest('Get My Rewards', false, `Has reward: ${hasRedeemedReward}, Has QR: ${allHaveQRCodes}`);
            }
        }
        catch (error) {
        ...
        try {
            ...
            const allActive = activeRewards.every((r) => r.status === 'active');
            logTest('Filter Redeemed by Status', allActive, `Found ${activeRewards.length} active rewards`);
        }
        catch (error) {
        ...
        try {
            await axios.post(`${BASE_URL}/api/rewards/invalid-id/redeem`, {}, {
                headers: { Authorization: `Bearer ${authToken}` },
            });
...
// Run tests
runTests();
//# sourceMappingURL=test-rewards.js.map