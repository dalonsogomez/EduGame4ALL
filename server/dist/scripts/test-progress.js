...
async function testProgressEndpoints() {
    ...
    try {
        ...
        try {
            ...
            if (!Array.isArray(badges)) {
                ...
                if (badges.length > 0) {
                    ...
                    const hasRequiredFields = badge.hasOwnProperty('_id') &&
            ...
            if (gamesResponse.data.games && gamesResponse.data.games.length > 0) {
                ...
                for (let i = 0; i < 2; i++) {
                    const sessionData = {
                        ...
                        maxScore: 10,
                        timeSpent: 300 + (i * 50),
                        answers: game.questions?.slice(0, 3).map((q, idx) => ({
        ...
        try {
            ...
            if (!Array.isArray(history)) {
                ...
                if (history.length > 0) {
                    ...
                    const hasRequiredFields = session.hasOwnProperty('id') &&
        ...
        try {
            ...
            // Check structure
            const hasRequiredFields = report.hasOwnProperty('gamesPlayed') &&
            ...
            const summaryValid = typeof report.gamesPlayed === 'number' &&
                typeof report.xpEarned === 'number' &&
                typeof report.totalTime === 'number' &&
                typeof report.avgAccuracy === 'number';
            if (summaryValid) {
                logResult('Get Weekly Report - Summary', true, `Games: ${report.gamesPlayed}, XP: ${report.xpEarned}, Time: ${report.totalTime}min, Accuracy: ${report.avgAccuracy}%`);
            }
            else {
            ...
            if (Array.isArray(report.strengths) && Array.isArray(report.improvements) && Array.isArray(report.insights)) {
                logResult('Get Weekly Report - AI Insights', true, `Strengths: ${report.strengths.length}, Improvements: ${report.improvements.length}, Insights: ${report.insights.length}`);
            ...
            if (Array.isArray(report.weeklyActivity) && report.weeklyActivity.length === 7) {
                ...
                const activityValid = report.weeklyActivity.every((day) => day.hasOwnProperty('day') &&
            ...
            if (report.categoryBreakdown) {
                const hasCategories = report.categoryBreakdown.hasOwnProperty('language') &&
